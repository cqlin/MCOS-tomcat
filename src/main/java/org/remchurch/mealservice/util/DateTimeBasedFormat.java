...
public final class DateTimeBasedFormat {
	...
	private static final String DATE_SIMPLE = "yyyy-MM-dd";
    ...
    private static final DateTimeFormatter FORMAT_SIMPLE;
    ...
    static {
    	FORMAT_SIMPLE = DateTimeFormatter.ofPattern(DATE_SIMPLE, Locale.US);
    ...
    public static final String getCurrentDate() {
        final ZonedDateTime now = ZonedDateTime.now(ZoneId.of("US/Eastern"));
        final LocalDateTime old = CURRENT_DATE_GENERATED.get();

        if (old.until(now, ChronoUnit.MILLIS) > 1000) {
            currentDate = FORMAT_SIMPLE.format(now);
