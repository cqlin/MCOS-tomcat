<%--
    ...
    Note: block tag is invoked when the jsp is rendered, by include or other means.
    and the attributes will be evaluated before the tag code is called. 
    The child code is called before parent code is called. so child attributes is set first.
    But included page in attribute is called before the main page is called, so if included page
    has block main template, it will be evaluated first, so the attribute in the included page 
    is set first.  If need to override this, then need to explicitly do it in the main page before
    block tag.
...
--------------------------------------------------
issues:
1. seems no jsp comment allowed in the main template before body html tag.  otherwise it gives:
 Caused by: org.apache.jasper.JasperException: /WEB-INF/templates/base.jsp (line: [7], column: [9]) 
 jsp:attribute must be the subelement of a standard or custom action
 This is because when using tag, it can only contain attributes and body tag, comments inside body tag and attribute tag is fine.
2. eclipse gives - Invalid text string syntax error if the attributes are directly in template jsp.
3. if wrap the attributes in <jsp:element name="elname"></jsp:element> tag, eclipse syntax is satisfied, but 
  the content is wrapped in <elname> tag in output.
4. cannot use attribute name that conflicts with implicit objects such as $header.
